...
from Env.models import EnvironmentVariable
...
def move(request):
    ...
    try:
      ...
      # back
      r, err = ssh.execCommand("mv " + ev.ftp_war_path + ev.ftp_war_name + " " + ev.ftp_war_back_path + ev.ftp_war_name[0:-4] + "_" + dateutils.getcurrentdate() + ".war")
      if err.strip is not None:
        raise Exception(err)
      result.append(r)
      result.append("\r\n")
      ...
      result.append("Exec completed!")
      logger.info("result : " + result)
    except Exception, e:
      logger.error(e)        #直接将错误写入到日志文件
      logger.info("view.move is end")
      return HttpResponse('Exec failure!(' + str(e) + ")")
    finally:
      if ssh is not None:
        ssh.close()
    logger.info("view.move is end")
...
def recovery(request):
    ...
    try:
      ...
      # back
      r,err = ssh.execCommand("rm -fr " + ev.ftp_war_path + ev.ftp_war_name)
      if err.strip is not None:
        raise Exception(err)
      r,err = ssh.execCommand("cp " + ev.ftp_war_back_path + ev.ftp_war_name[0:-4] + "_" + dateutils.getcurrentdate() + ".war" + " " + ev.ftp_war_path + ev.ftp_war_name)
      if err.strip is not None:
        raise Exception(err)
